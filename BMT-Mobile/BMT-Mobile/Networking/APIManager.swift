...
class ApiManager {
    ...
    func createImobile(imobile: Immobile, completion: @escaping (_ result: Bool, NSError?) -> Void) {
        let url = API.baseURL + API.imoveis
        let parameters: Parameters = ["title":imobile.title,"description":imobile.itemDescription,"img_url":imobile.imgURL]
        let headers: HTTPHeaders = [.authorization(bearerToken: SessionManagerUser.shared.token ?? "")]
